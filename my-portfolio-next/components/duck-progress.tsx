...
export function DuckProgress({ sections, currentSection }: DuckProgressProps) {
  ...
  useEffect(() => {
    setIsWaddling(true)
    const timeout = setTimeout(() => setIsWaddling(false), 500)
    return () => clearTimeout(timeout)
  }, [currentSection])

  if (!mounted) return null

  const duckPosition = totalSections > 1 ? (currentSection / (totalSections - 1)) * 100 : 0
      ...
      <div className="bg-card/95 backdrop-blur-md border-2 border-border rounded-full px-4 sm:px-6 py-2 sm:py-3 shadow-xl">
        <div className="relative w-48 sm:w-72 h-10 sm:h-12 flex items-center">
          ...
          <motion.div 
            ...
            initial={{ width: 0 }}
            animate={{ width: `calc(${duckPosition}% - 16px)` }}
          ...
          <motion.div
            ...
            animate={{
              left: `calc(${duckPosition}% * 0.85 + 7.5%)`,
              rotate: isWaddling ? [0, -15, 15, -15, 0] : 0,
              y: isWaddling ? [0, -4, 0, -4, 0] : 0,
            }}
            transition={{
          ...
          >
            <DuckIcon className="drop-shadow-lg" />
          </motion.div>
        </div>
